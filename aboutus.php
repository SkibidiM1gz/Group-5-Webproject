<?php
    require "./project.php";
?>
<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=1600, height=1200, initial-scale=1">
    <title>About Us | Golby's Pizzeria</title>
    <link rel="stylesheet" href="./theme.css" />
    <link rel="stylesheet" href="./layout.css" />
    <!--<link rel="stylesheet" href="./components.css" />-->
    <style type="text/css">
        /* body {
            border: 10px solid #b04a44;
            margin: 0;
            padding: 20px;
            background-color: #3b1c1a;
            font-family: Arial, sans-serif;
        } */
        
        :root {
            --theme-color-light1: #f4d4a0;
            --theme-color-light2: #ffb19a;
            --theme-color-light3: #ffe0d4;
            
            --theme-color-dark1: #b04a44;
            --theme-color-dark2: rgba(255, 140, 100, 0.6);
            --theme-color-dark3: rgba(176, 74, 68, 0.4);
            --theme-color-dark4: rgba(59, 28, 26, 0.3);
            --theme-color-dark5: #2b1b18;
            
            --aboutus-contentbox-border: 15px solid var(--theme-color-brick);
            --aboutus-contentbox-background: var(--theme-color-light);
            --aboutus-heading-text: var(--theme-color-cocoa);
            --aboutus-heading-title: var(--theme-color-light);
            --aboutus-table-text: var(--theme-color-cocoa);
        }
        
        
        h2,
        h3 {
            color: var(--aboutus-heading-text);
            text-align: center;
            /*color: var(--theme-color-text-ondark);*/
            /* light1 */
        }
        
        h1 {
            font-size: 46px;
            font-weight: bold;
            letter-spacing: 3px;
            margin-top: 4vh;
            margin-bottom: 2vh;
            color: var(--aboutus-heading-title);
            text-align: center;
            /*transition: 0.3s;*/
        }
        
        /* h1:hover {
            transform: scale(1.1);
        } */
        
        /*h3 {
            transition: color 0.3s ease, letter-spacing 0.3s ease, transform 0.3s ease;
        }*/
        
        /* h3:hover {
            color: var(--theme-color-text-hover-ondark); light2
            cursor: default;
            transform: scale(1.1);
            letter-spacing: 2px;
        } */
        
        /*main th,
        main td,
        p {
            font-size: 1rem;
        }*/
        
        .content-box {
            width: 60vw;
            margin: auto;
            background-color: var(--aboutus-contentbox-background);
            border-left: var(--aboutus-contentbox-border);
            border-right: var(--aboutus-contentbox-border);
            border-radius: 9px;
            
            padding: 3vh 4vw;
            
            /*padding-left: 4vw;
            padding-right: 4vw;
            padding-top: 3vh;
            padding-bottom: 3vh;*/
            margin-bottom: 3vh;
        }
        
        .content-box p {
            margin: auto;
            text-align: justify;
            word-spacing: 0.3rem;
            line-height: 2.5rem;
        }
        
        .content-box .center {
            text-align: center;
        }
        
        .content-box * {
            margin-left: 4vw;
            margin-right: 4vw;
        }
        
        /* main p {
            text-align: center;
            border: 2px solid var(--theme-color-border-dark);
            dark1
            padding: 10px;
            margin: 30px;
            border-radius: 10px;
            background-color: var(--theme-color-background-light);
            light3
            box-shadow: 0 4px 8px var(--theme-color-shadow-dark);
            dark4
            color: var(--theme-color-text-onlight);
            dark5
            transition: box-shadow 0.3s ease, transform 0.3s ease;
        } */
        
        /* main p:hover {
            box-shadow: 0 0 20px 4px var(--theme-color-shadow-hover-dark); dark2
            transform: scale(1.02);
        } */
        
        /* main .authors-box p:hover {
            box-shadow: none;
            transform: none;
        } */
        
        /* .authors-box {
            border: 2px solid var(--theme-color-border-dark);
            dark 1
            border-radius: 12px;
            box-shadow: 0 4px 10px var(--theme-color-shadow-dark);
            dark3
            width: 75vw;
            background-color: var(--theme-color-background-light);
            light3
            padding: 20px;
            margin: 20px auto;
            transition: box-shadow 0.3s ease, transform 0.3s ease;
        } */
        
        /* .authors-box:hover {
            box-shadow: 0 0 25px 6px var(--theme-color-shadow-dark);dark2
            transform: scale(1.02);
        } */
        
        /*.authors-box p {
            border: none;
            box-shadow: none;
            margin: 5px 0;
        }*/
        
        /* .navbar {
            background-color: #b04a44;
            padding: 10px 0;
            border-radius: 0 0 10px 10px;
            text-align: center;
        } */
        
        /* .navbar a {
            color: #f4d4a0;
            font-weight: bold;
            text-decoration: none;
            margin: 0 15px;
            transition: color 0.3s ease;
        } */
        
        /* .navbar a:hover {
            color: #fff8f6;
        } */
        
        /* .logo-container img {
            width: 100px;
            height: 100px;
        } */
        
        /* .contact-section {
            width: 75%;
            margin: 0 auto;
            text-align: center;
        } */
        
        .authors-list {
            /*width: 60%;*/
            width: 100%;
            margin: 2rem auto;
            border-collapse: collapse;
        }
        
        .authors-list th,
        .authors-list td {
            padding: 0.5rem 0.7rem;
            color: var(--aboutus-table-text);
            /*var(--theme-color-text-onlight);*/
            /* dark5*/
            font-size: 1.2rem;
        }
        
        img.content-logo {
            display: block;
            width: 20vmin;
            height: auto;
            margin: 5vh auto;
        }
        
        /*section {
            width: 75vw;
            margin: auto;
        }
        
        section.lore p {
            text-align: justify;
        }*/
    </style>
</head>

<body>
    <header>
        <?php
            require "./navbar.php";
        ?>
        <!-- <img class="logo" src="./images/logo.png">
        <nav>
            <ul>
                <li><a href="index.html">Home</a></li>
                <li class="active-page"><a href="aboutus.html">About Us</a></li>
                <li><a href="faq.html">FAQ</a></li>
                <li><a href="cart.html">Cart</a></li>
                <li><a href="products/products.html">Products</a></li>
                <li><a href="login.html">Login</a></li>
                <li><a href="user_account.html">Account</a></li>
            </ul>
        </nav> -->
    </header>
    <main>
        <!-- header -->
        <!-- <table width="100%">
            <tr>
                <td align="center"> -->
        <!--<h1>Golby's Pizzeria</h1>-->
        
        <h1 class="page-title">About Us</h1>
        <img class="content-logo" src="./images/logo.png" />
        <article id="article-about">
            <section id="section-lore">
                <div class="content-box">
                    <h2>Background</h2>
                    <p>At Golby's Pizzeria, we take pride in serving exceptional pizza crafted with love, tradition, and freshly harvested ingredients. Our commitment to quality begins with our handmade dough prepared fresh daily, followed by carefully selected toppings and home-made sauces.
                        Whether you're ordering for a family gathering, a business lunch, or a relaxing evening at home, Golby's Pizzeria offers a smooth online ordering experience and reliable delivery.</p>
                </div>
            </section>
            <section id="section-details">
                <div class="content-box">
                    <h2>Details</h2>
                    <p>We open only from Monday to Friday, 7am to 5pm.</p>
                    
                    <p>You can find us near Misamis Street, Bago Bantay, Quezon City, Philippines.</p>
                    
                    <p>For any inquiries contact us via:<br>
                        E-mail: golbyspizzeria@gmail.com<br>
                        Phone: +63 961 825 0366
                    </p>
                </div>
            </section>
            <section id="section-authors">
                <div class="content-box">
                    <h2>Authors</h2>
                    <!--class="authors-box" -->
                    <p>An E-Commerce pizza shop website made by Group 5 with the contributions of:</p>
                    <table class="authors-list">
                        <tr>
                            <th>Shiela Mae M. Alcantara</th>
                            <td>- Asset Research/Content</td>
                        </tr>
                        <tr>
                            <th>Daniel Derrick M. Calisura</th>
                            <td>- Programming/Content</td>
                        </tr>
                        <tr>
                            <th>Franco Miguel Dorato</th>
                            <td>- Programming</td>
                        </tr>
                        <tr>
                            <th>John Gabriel Geraldizo</th>
                            <td>- Asset Research/Content</td>
                        </tr>
                    </table>
                </div>
                <!--</div>-->
            </section>
        </article>
        <article id="article-credits">
            <section id="section-credits">
                <div class="content-box">
                    <h2>Credits</h2>
                    <!--<h3>Images</h3>-->
                    <p>Logo created by Franco Miguel Dorato's brother</p>
                    <p>All product images taken from pinterest.com</p>
                    <!--<h3>Font</h3>-->
                    <p>Fonts used Montserrat |<a href="./fonts/montserrat/OFL.txt">license</a></p>
                    <!--<section id="section-images">-->
                    <!--</section>
                <section id="section-fonts">-->
                    <!--</section>-->
                </div>
            </section>
        </article>
        
        <!-- </td>
                            </tr>
                        </table> -->
        
        <!-- about us -->
        
        
        <!-- contact Info -->
        <!-- <div class="contact-section">
            
        </div> -->
        
        <!-- authors -->
        
    </main>
    <footer>
        <p class="footer-content">&copy;Golby's Pizzeria 2025</p>
        <!-- <p class="footer-content">Contact us: golbyspizzeria@gmail.com | +639618250366</p> -->
    </footer>
    
    
    <!-- Navbar -->
    <!-- <div class="navbar">
        <div class="logo-container">
            <img src="./images/logo.png">
        </div>
        <div class="link-container">
            <a href="aboutus.html">About Us</a>
            <a href="faq.html">FAQ</a>
            <a href="cart.html">Cart</a>
            <a href="login.html">Login</a>
            <a href="user_account.html">Account</a>
        </div>
    </div> -->
    
    <!-- header -->
    <!-- <table width="100%">
        <tr>
            <td align="center">
                <img src="images/logo.png" width="100" height="100" />
                <h1>Golby's Pizzeria</h1>
            </td>
        </tr>
    </table> -->
    
    <!-- about us -->
    <!-- <h2 align="center">About Us</h2>
    <p>At Golby's Pizzeria, we take pride in serving exceptional pizza crafted with love, tradition, and freshly harvested ingredients. Our commitment to quality begins with our handmade dough prepared fresh daily, followed by carefully selected toppings and home-made sauces.
        Whether you're ordering for a family gathering, a business lunch, or a relaxing evening at home, Golby's Pizzeria offers a smooth online ordering experience and reliable delivery.</p> -->
    
    <!-- contact Info -->
    <!-- <div class="contact-section">
        <h3>Business Days</h3>
        <p>We open only from Monday to Friday, 7am to 5pm.</p>
        
        <h3>Location</h3>
        <p>You can find us near Misamis Street, Bago Bantay, Quezon City, Philippines.</p>
        
        <h3>Contact</h3>
        <p>Email: <b>golbyspizzeria@gmail.com</b><br>Phone: <b>+63 961 825 0366</b></p>
    </div> -->
    
    <!-- authors -->
    <!--     <h2 align="center">Authors</h2>
<div class="authors-box">
    <p>An E-Commerce pizza shop website made by Group 5 with the contributions of:</p>
    
    <table class="authors-list">
        <tr>
            <td><b>Shiela Mae M. Alcantara</b></td>
            <td>- Asset Research/Content</td>
        </tr>
        <tr>
            <td><b>Daniel Derrick M. Calisura</b></td>
            <td>- Programming/Content</td>
        </tr>
        <tr>
            <td><b>Franco Miguel Dorato</b></td>
            <td>- Programming</td>
        </tr>
        <tr>
            <td><b>John Gabriel Geraldizo</b></td>
            <td>- Asset Research/Content</td>
        </tr>
    </table>
</div> -->
</body>

</html>